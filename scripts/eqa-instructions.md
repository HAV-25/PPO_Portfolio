...
MOONSHOT_API_KEY=sk-4iNxi7DW3SIxFUwUDnfF5XnofARTCZNeFnP1BMbqHGbOQBjy
```
...
### 4B-3: Revise the draft via Kimi API

Revisions are generated by calling the Moonshot AI (Kimi) API.
You (Claude) construct the targeted revision prompt; Kimi produces the revised content.
...
Construct the revision prompt and write it to /tmp/kimi-revision-prompt.txt:

```
You are revising a professional article written for Payal Ponkshe (former Mastercard VP,
fintech/payments/AI expert). Make ONLY the targeted changes listed below — do not
rewrite sections that are not listed as failures.

ORIGINAL ARTICLE:
[paste full MDX content of the draft]

REVISION INSTRUCTIONS (fix ONLY these items):
[paste the revision_instructions array from the feedback object, one item per line]

VOICE REFERENCE (match this register for any rewritten paragraphs):
- Data-first, no preamble, sentence case throughout
- Numbers in numerals, inline citations as (Source Name, Month Year)
- Revenue Per Employee as commercial frame
- No buzzwords: transformative, innovative, game-changing, seamless, revolutionary
- Builder credibility: first-person references to specific tech stack where relevant
- Paragraphs max 2 lines

OUTPUT: Return the complete revised MDX article (frontmatter + body).
If the LinkedIn post needs revision (items 7 or 14 failed), write ===LINKEDIN===
on a new line after the article and provide the revised LinkedIn post (150–250 words).
If the LinkedIn post does NOT need revision, do not include the ===LINKEDIN=== section.
```
...
const prompt = require('fs').readFileSync('/tmp/kimi-revision-prompt.txt', 'utf8');
...
const req = https.request(options, res => {
  ...
  res.on('end', () => {
    ...
    require('fs').writeFileSync('/tmp/kimi-revision.txt', parsed.choices[0].message.content);
    console.log('Kimi revision complete. Tokens used:', parsed.usage?.total_tokens || 'unknown');
  });
});
...
Parse /tmp/kimi-revision.txt:
- Everything before `===LINKEDIN===` (or the whole file if delimiter absent) → revised article MDX
- If `===LINKEDIN===` present, everything after → revised linkedin_post (update .meta.json)
